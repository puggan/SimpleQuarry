		...
		extends ScreenWTFMojang<ContainerPoweredQuarry>
{
	...
	protected boolean renderForeground(PoseStack matrix, int mx, int my)
	{
		if(mx - leftPos >= 6 && my - topPos >= 7 && mx - leftPos <= 19 && my - topPos <= 73)
		{
			...
			if(mouse.isEmpty())
				renderTooltip(matrix, Component.literal(df.format(Math.floor(tile.storage.getStoredQF(null) / (UniversalConverter.FT_QF(ForgeHooks.getBurnTime(SQCommonProxy.COAL, null)) / SQConfig.getBlockPerCoal()))) + " " + I18n.get("info.squarry.blockstobreak")), 16, 48);
		}
